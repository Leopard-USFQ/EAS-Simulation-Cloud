...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    // Create the solid world volume as a parallelepiped
    G4Box *solidWorld = new G4Box("solidWorld", worldSizeX / 2, worldSizeY / 2, worldSizeZ / 2);

    // Create the logical world volume using air material
    logicWorld = new G4LogicalVolume(solidWorld, worldMat, "logicWorld");

    // Place the logical world volume inside the physical world volume
    G4VPhysicalVolume *physWorld = new G4PVPlacement(0, G4ThreeVector(0.,0.,0.), logicWorld, "physWorld", 0, false, 0, true);

    // Define the phantom detector material as air
    G4Material *ghostDetectorMat = nist->FindOrBuildMaterial("G4_AIR");


    // Create the solid volume of the phantom detector as a parallelepiped
    G4Box *solidDetector = new G4Box("solidDetector", detectorSizeX / 2, detectorSizeY / 2, detectorSizeZ / 2);

    // Create the logical volume of the phantom detector using air material
    logicDetector = new G4LogicalVolume(solidDetector, ghostDetectorMat, "logicDetector");

    // Place the logical volume of the phantom detector inside the logical world volume
